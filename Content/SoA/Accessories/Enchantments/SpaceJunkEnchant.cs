...
namespace CSE.Content.SoA.Accessories.Enchantments
{
    ...
    public class SpaceJunkEnchant : BaseEnchant
    {
            ...
            public override void OnHitByEither(Player player, NPC npc, Projectile proj)
            {
                if (Main.rand.NextFloat() < 0.33f)
                {
                    ...
                    if (player.whoAmI == Main.myPlayer)
                    {
                        for (int i = 0; i < 4; i++)
                        {
                            int dmg = (int)player.GetDamage<GenericDamageClass>().ApplyTo(player.HasEffect<FoundationsEffect>() ? 400 : 40);
        ...
        public class SpaceJunkAbilityEffect : AccessoryEffect
        {
            ...
            {
                CooldownBarManager.Activate("SpaceJunkEnchantCooldown", ModContent.Request<Texture2D>("CSE/Assets/Textures/Content/Items/Accessories/Enchantments/SpaceJunkEnchant").Value, new(237, 73, 78),
