    ...
    public class ConjuristSoulRecipe : ModSystem
    {
        public override void PostAddRecipes()
        {
            for (int i = 0; i < Recipe.numRecipes; i++)
            {
                ...
                {
                    if (ModCompatibility.SacredTools.Loaded) { recipe.AddIngredient(ModCompatibility.SacredTools.Mod.Find<ModItem>("StardustSigil"), 1); recipe.AddIngredient(ModCompatibility.SacredTools.Mod.Find<ModItem>("StarstreamVeil"), 1); recipe.RemoveIngredient(ItemID.MonkBelt); recipe.RemoveIngredient(ItemID.SquireShield); recipe.RemoveIngredient(ItemID.HuntressBuckler); recipe.RemoveIngredient(ItemID.ApprenticeScarf); recipe.RemoveIngredient(ItemID.SummonerEmblem); }
    ...
    public class ConjuristSoulEffects : GlobalItem
    {
        ...
        public override void ModifyTooltips(Item item, List<TooltipLine> tooltips)
        {
            ...
            if (item.type == ModContent.ItemType<ConjuristsSoul>() )
            {
                ...
                if (ModCompatibility.Homeward.Loaded)
                {
                    tooltips.Insert(8, new TooltipLine(Mod, "mayo1", Language.GetTextValue(key + "HWJConjurist")));
                }
                if (ModCompatibility.SacredTools.Loaded)
                {
                    tooltips.Insert(8, new TooltipLine(Mod, "mayo2", Language.GetTextValue(key + "SoAConjurist")));
                }
            }
            if (ModCompatibility.Homeward.Loaded)
            {
